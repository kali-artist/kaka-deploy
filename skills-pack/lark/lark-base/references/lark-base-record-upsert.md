...
- 🔴 **`--json` 不要包 `fields` 外壳**：直接传 `{"字段名": 值}`，不要传 `{"fields": {"字段名": 值}}`。后者报 `800010701 Record write payload must not be wrapped in fields`。这与 `+record-batch-create` 的 `{"fields": [...], "rows": [...]}` 格式不同，容易混淆。
