...
| 查看/修改身份策略 strict-mode | `lark-cli config get strict-mode`；`lark-cli config set strict-mode off\|bot\|user`（详见下文"strict-mode 硬门禁"） |
...
### strict-mode 硬门禁（诊断 auth login/身份缺失时先查这个）

lark-cli 有一个全局身份策略 `strict-mode`，会**硬性阻断另一身份的所有命令**，早于 scope 检查执行。诊断"user 身份缺失 / auth login 报错 / whoami 只见 bot 不见 user"时**必须先查这里**，否则会误判成 scope 或登录问题：

| strict-mode 值 | 行为 |
|---|---|
| `off` | 两种身份都可用（默认推荐） |
| `bot` | 只允许 bot 命令，`auth login` 及所有 `--as user` 命令一律被拒 |
| `user` | 只允许 user 命令，屏蔽所有 bot 操作 |

**症状** → `auth login` 直接报错、`whoami` 里 user 显示 missing、看似"user 没登录"但其实是被门禁拦住。

**排查命令**：`lark-cli config get strict-mode`。若为 `bot` 且需要 user 能力，用 `lark-cli config set strict-mode off`（或先 off → 登录 user → 需要时再切回 bot）。**切换 strict-mode 属于安全策略变更，必须先向用户明确说明当前值、建议值和风险，得到显式同意再改**，不要静默切换。
