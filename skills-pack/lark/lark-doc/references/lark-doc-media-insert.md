...
# 文件渲染样式：card（默认卡片）| preview（音视频内联播放器）| inline
lark-cli docs +media-insert --doc doxcnXXX --file ./report.pdf --type file --file-view card

# 🔴 --file 必须用相对路径！绝对路径会被拒绝。先 cd 到文件所在目录：
cd /path/to/file && lark-cli docs +media-insert --doc doxcnXXX --file ./report.pdf --type file

# 定位插入（默认追加到文档末尾，用 --selection-with-ellipsis 可插入到指定段落旁边）
# 匹配到该文本的 block 后面插入；多个匹配时用 'start...end' 消歧
cd /tmp && lark-cli docs +media-insert --doc doxcnXXX --file ./pack.zip --type file \
  --selection-with-ellipsis "下载附件...部署步骤"
...
| `--file <path>` | 二选一 | 本地文件路径（文件大于 20MB 时自动切换分片上传）。**🔴 必须用相对路径，绝对路径会被拒绝！先 `cd` 到文件目录再用 `./filename`** |
| `--type <type>` | 否 | `image`（默认）或 `file`。`--from-clipboard` 目前只产出 image。 |
| `--file-view <view>` | 否 | 仅 `--type=file`：`card`（默认卡片）| `preview`（音视频内联播放器）| `inline` |
| `--selection-with-ellipsis <text>` | 否 | 定位插入：匹配文档中的文本，在其 block 后面插入（默认追加到文档末尾）。多个匹配时用 `start...end` 消歧 |
