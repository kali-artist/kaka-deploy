...
- **表格行增删必须用 block_replace（⚠️ 常见陷阱）**：`str_replace` 无法向已有表格添加新行——replacement 中的 `<tr>`/`<td>` 等标签会被当作行内文本而非 XML 结构，导致内容混入相邻单元格或标题。正确做法：
  1. `docs +fetch --detail with-ids` 获取表格的 block ID（`<table id="doxcn...">`）
  2. 从返回 XML 中提取完整表格结构（含 `<colgroup>`、`<thead>`、所有 `<tbody>` 行）
  3. 用 `block_replace --block-id <table_id> --content '<table>...完整表格XML含新增行...</table>'` 整体替换
  4. 重建时保留原有列宽、表头和所有已有行，仅追加/修改目标行
  5. 注意：`block_replace` 后旧 block ID 失效，后续操作需重新 fetch
