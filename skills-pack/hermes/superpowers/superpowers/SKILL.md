...
description: >
  ...
  Requires terminal/execute_code tools and delegate_task subagents in Hermes Agent.
---
...
- After saving, offer two execution modes:
  - **Subagent-driven (current session):** `delegate_task` per task + two-stage review
...
**Per-task loop (Hermes):**
1. `delegate_task` an implementer subagent with task + full plan context
2. Wait for completion announcement
3. `delegate_task` a spec-reviewer subagent → must confirm code matches spec
4. `delegate_task` a code-quality reviewer subagent → must approve quality
...
## Hermes Subagent Dispatch Pattern

When dispatching implementer or reviewer subagents, use `delegate_task`:
...
Run `delegate_task` with the task as a detailed prompt. The sub-agent announces results automatically.
