...
Adapted for Hermes Agent (`delegate_task` instead of Claude Code Task tool / OpenClaw `sessions_spawn`)
...
Use `delegate_task` for each role. Pass a structured prompt:
...
1. **Dispatch implementer sub-agent** via `delegate_task`
   - Include: full task text, plan file path, TDD constraint
   - Wait for completion announcement

2. **Dispatch spec-reviewer sub-agent** via `delegate_task`
...
3. **Dispatch code-quality reviewer sub-agent** via `delegate_task`
