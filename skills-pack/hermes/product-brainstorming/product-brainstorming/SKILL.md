...
- **Execution baseline after convergence**: when a brainstorm has converged into an agreed structure, template, plan, or decision, that captured convergence becomes the execution baseline. If the user later says “执行/按这个来/继续做/落地/固化”, execute against the converged brainstorm output instead of re-interpreting, redesigning, or substituting a new approach. Only deviate if the user explicitly changes direction or asks to re-brainstorm.
- If later asked to review the brainstorm, first inspect the actual conversation/session record for that brainstorm when available. Do not reconstruct from vague memory, cross-session summaries, or assumptions. Then reconstruct the thinking trace: which brainstorming mode(s) were used, what assumptions were challenged, how the conversation moved from framing → divergence/provocation → convergence, and what concrete direction or skill/product structure resulted. Also explicitly distinguish (a) skills/tools actually used during the original brainstorm, (b) skills loaded later only to review or reconstruct the brainstorm, and (c) inferred frameworks that shaped the answer but were not actually invoked. If the original record is unavailable or compressed beyond recovery, say that explicitly and separate confirmed facts from inference.
